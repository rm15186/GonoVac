...
classdef VacAMR_IBM3HighPrev < handle
    properties
        ...
        vac_state;          % vaccinated individuals (hopefully over time!)
        vaccinated_since    % day of most recent vaccination
...
%             counters.n_traced;           % number of people traced at each time step
        ...
        methods
            ...
            function [self] = VacAMR_IBM3HighPrev(N, params, adj_set, VERBOSE, LOW_MEM, vac)
                ...
                self.indiv_state = false(self.N,self.n_Strains,2,1); 
                size(self.indiv_state); %1000 2 1
                
                % Vac state
                self.vac_state = false(self.N,1); %initially no one is vaccinated
                
                self.vaccinated_since = nan(self.N,1);
...
%                 end
              ...
              %% Infect population on day zero
                ...
                self.indiv_state(id_toinfect,1,1,1) = 1; %infect people with those ids with nonAMR
                ...
                % for each strain
                self.infected_since(self.indiv_state(:,1,1,1)==1,1) = 0;
                self.infected_since(self.indiv_state(:,2,1,1)==1,2) = 0;
                self.counters.infection_count(self.indiv_state(:,1,1,1)==1,1) = 1;
                self.counters.infection_count(self.indiv_state(:,2,1,1)==1,2) = 1;
                
                % prescribe proportion of symptomatic infections
                %size(self.indiv_state) 1000 2 2
                idx_infected = any(self.indiv_state,2); %dim2 is strain so if there is a 1 in either strain
                idx_infected = idx_infected(1); %1 is the disease, 2 is the vaccine
            ...
            function [self] = simulate(self,n_Days)
              %% Continue SIS simulation from where we left off
                ...
                %should have a burn in days parameter not 2000, maybe 2000
                %wont work
                self.counters.burn_in_prevalence = cat(1,self.counters.burn_in_prevalence,zeros(3000,self.n_Strains));
                self.counters.burn_in_prevalence_either = cat(1,self.counters.burn_in_prevalence_either,zeros(3000,1));
                ...
                while self.today <= start_day + n_Days - 1 
                  ...
                  %% infection of strain susceptible individuals (zeros in state matrix)          
                    new_state = self.spread_infection(current_state, new_state, current_vac_state);
                ...
                %protection
                self.counters.current_vac(self.today+1) = sum(self.vac_state(:,self.today)~= 0); %works now index in pos 2 invalid - burn in???
                  ...
                  %% Burn-in (if selected)
                    ...
                    if self.burn_in == true
                        ...
                        %after weve burned in we want to store the burn in
                        %prevlaences to plot a steady state before we
                        %introduce the vaccine
                        self.counters.burn_in_prevalence(self.today+1,:) = sum(self.indiv_state(:,:,self.today+1),1);
                        self.counters.burn_in_prevalence_either(self.today+1) = sum(any(self.indiv_state(:,:,self.today+1),2),1);
                        %burn in for 3000 days, looks about right
                        if  self.today == 2999 %set the values here for the real simulation
            ...
            function [new_state] = spread_infection(self, current_state, new_state, current_vac_state)
...
%                 current_state = self.indiv_state(:,:,end);
                ...
                %TODO vaccinated people less likely to get symptoms
...
%                 h_traced_AMR = plot(days,data.n_attended_traced(days+1,3),'r-','linewidth',2);
            ...
            function [fig_h, plot_names] = plot_doses(data, ave, DIR_NAME)
              %% Plot number of doses of each drug and their effectiveness
                ...
                % SAVE PLOT
                if ~isempty(DIR_NAME)
                    % check output folders exist, if not then create[h_fig_dose,plot_names] = VacAMR_IBM2.plot_doses(data, N, ave, DIR_NAME);
                    if exist('./outdata','dir')~=7;mkdir('./outdata');end
                    if exist(['./outdata/',DIR_NAME],'dir')~=7;mkdir(['./outdata/',DIR_NAME]);end
