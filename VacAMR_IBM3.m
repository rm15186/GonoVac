        ...
        methods
            ...
            function [self] = VacAMR_IBM3(N, params, adj_set, VERBOSE, LOW_MEM)
                ...
                self.counters = struct;
                    ...
                    self.counters.vaccinated = 0;
                    %self.counters.vaccinated_new = 0; maybe?
            ...
            %% Vaccinate on day 0? for childhood vaccines?
            ...
            function [self] = simulate(self,n_Days)
              %% Continue SIS simulation from where we left off
                ...
                self.counters.vaccinated = cat(1,self.counters.vaccinated,zeros(n_Days,1));
                %self.counters.vaccinated_new = cat(1,self.counters.vaccinated_new,zeros(n_Days,1))
...
%                 current_state = self.indiv_state(:,:,end);
                ...
                % today
                %size(rand(self.N,self.n_Strains))
                double_vac_state = [current_vac_state,current_vac_state]
                
                %size(double_vac_state)
                %size(current_vac_state)
                %max(rand(self.N,self.n_Strains),double_vac_state)
                ...
                idx_infected_today = any(idx_infect,2); %RM fucked up by vac dim?
