...
classdef VacAMR_IBM < handle
    ...
    properties
        ...
        %EPSILON        % percentage getting vaccinated when offered
        %ZETA           % rate of loss /chance of losing protection
...
%             counters.n_traced;           % number of people traced at each time step
        ...
        methods
            ...
            function [self] = AMR_IBM(N, params, adj_set, VERBOSE, LOW_MEM)
                ...
                % 1: non-AMR state
                % 2: AMR state
                % Binary if theyre vaccinated or not
                % 3: date infection acquired (either strain)
                % 4: symptomatic (true / false)
                self.indiv_state = false(self.N,self.n_Strains+1,1); %+1 for vaccinated or not
              ...
              %% Infect population on day zero
                ...
                %RM randomly vaccinate some people for childhood/ just to
                %initialise dont vac any
                idx_vac = randperm(self.N, round(self.p0(1)*self.N));
...
%                 current_state = self.indiv_state(:,:,end);
                ...
                if self.ALLOW_COINFECTION
                    ...
                    idx_not_infected = ~any(current_state,2); %if there is an infection in either current state, this includes the vaccination
                    mask = zeros(self.N,2);
